...
|---|---:|---|---|
| Agentic Reasoning & Tool Use | 15 | Plan-and-Execute, OpenAI function calling, real SQLite-backed tools, shared state | `LIVE_OPENAI_SUCCESS=true` |
| Graph-Based Orchestration | 20 | LangGraph StateGraph, nodes/edges, conditional branches, bounded re-plan loop | `REPLAN_LOOP_SUCCESS=true` |
...
| Persistence, HITL & Cloud | 20 | SQLite saver, interrupt/resume, restart-safe thread, FastAPI, Docker Compose | `HITL_RESTART_RESUME_SUCCESS=true` and `DOCKER_HEALTH_SUCCESS=true` |
| Documentation & Evidence | 5 | README, architecture, traceability, evidence directory, program attribution | All final evidence present and gate passes |
